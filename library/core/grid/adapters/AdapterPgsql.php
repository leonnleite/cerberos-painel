 ...
 * @version 0.0.0
 */
class AdapterPgsql extends \br\com\cf\library\core\model\ModelAbstract implements Grideable
{
        ...
        if ($grid->getParams('sSearch') != '') {
            ...
            for ($i = 0; $i < count($grid->getColumns()); $i++) {
                if ($grid->getParams('bSearchable_' . $i) == 'true') {
                    $where = $grid->getWhere();
                    $where .= sprintf("%s ilike '%s%s%s' or ", $grid->getColumns($i), '%', $grid->getParams('sSearch'), '%');
                    $grid->setWhere($where);
                }
            }
    ...
    {
        if ($grid->getWhere() != '') {
            $sQuery = sprintf('select count(*) as total from %s %s %s', $grid->getQuery(), $grid->getWhere(), $grid->getLimit());
            $stmt = $this->_conn->prepare($sQuery);
            $stmt->execute();
